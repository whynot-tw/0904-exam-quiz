...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {section === "wrong" && <WrongPanel rows={wrong.data ?? []} filter={wrongFilter} setFilter={setWrongFilter} onStart={(q: any) => { setMode("wrong"); setActiveQuestions([q]); setCursor(0); setAnswers([]); setSelected(null); setFinished(false); setSection("quiz"); }} />}
      {section === "stats" && <StatsPanel attempts={history.data ?? []} />}
...
function WrongPanel({ rows, filter, setFilter, onStart }: any) { const visible = rows.filter((r: any) => filter === "全部" || r.status === filter); return <div className="panel-wrap"><div className="section-heading"><div className="eyebrow">錯題本</div><h2>把錯的，變成熟悉。</h2><p>真正的知識錯題會留在這裡；閱讀失誤不會混在一起。</p></div><div className="filter-pills">{["待複習", "已熟悉", "全部"].map(f => <button className={filter === f ? "selected" : ""} onClick={() => setFilter(f)} key={f}>{f}</button>)}</div>{visible.length ? visible.map((r: any) => <Card className="wrong-row" key={r.id}><CardContent><div><Badge>{r.status}</Badge><h3>{r.questionId}</h3><p>錯誤 {r.wrongCount} 次 · 連續答對 {r.consecutiveCorrect} 次</p></div><Button variant="outline" onClick={() => onStart({ id: r.questionId })}>重刷</Button></CardContent></Card>) : <div className="empty-state"><RotateCcw size={28}/><h3>目前沒有符合條件的錯題</h3><p>完成幾回練習後，這裡會自動整理你的複習清單。</p></div>}</div> }
function StatsPanel({ attempts }: any) { return <div className="panel-wrap"><div className="section-heading"><div className="eyebrow">統計</div><h2>看見自己的進步。</h2><p>歷次測驗紀錄會在完成後自動同步。</p></div>{attempts.length ? attempts.map((a: any) => <Card className="history-row" key={a.id}><CardContent><span>{a.mode === "mock" ? "模擬考" : "練習"}</span><strong>{a.score} 分</strong><small>{a.correctCount}/{a.questionCount} 題答對</small></CardContent></Card>) : <div className="empty-state"><History size={28}/><h3>尚未有測驗紀錄</h3><p>完成第一回測驗後，這裡會顯示你的歷史成績。</p></div>}</div> }
function AdminPanel({ isAdmin }: { isAdmin: boolean }) { const [reviewOnly, setReviewOnly] = useState(true); const query = trpc.quiz.adminList.useQuery({ needsReviewOnly: reviewOnly }, { enabled: isAdmin }); if (!isAdmin) return <div className="empty-state"><ShieldCheck size={30}/><h3>需要 admin 權限</h3><p>此區域不對一般使用者開放。</p></div>; return <div className="panel-wrap"><div className="section-heading"><div className="eyebrow">Admin only</div><h2>題庫管理</h2><p>優先處理待審題目；官方答案仍以 PDF Source of Truth 為準。</p></div><div className="admin-toolbar"><Button variant={reviewOnly ? "default" : "outline"} onClick={() => setReviewOnly(true)}>只看待審</Button><Button variant={!reviewOnly ? "default" : "outline"} onClick={() => setReviewOnly(false)}>全部題目</Button><Badge>{query.data?.length ?? 0} 題</Badge></div><div className="admin-list">{query.data?.slice(0, 80).map((q: any) => <Card key={q.id} className="admin-row"><CardContent><div><Badge className={q.needsReview ? "review-badge" : ""}>{q.needsReview ? "待審" : "已匯入"}</Badge><h3>{q.id} · {q.text.slice(0, 80)}{q.text.length > 80 ? "…" : ""}</h3><p>正解：{q.correctOption} · {q.section}</p></div><Button variant="outline" size="sm">編輯</Button></CardContent></Card>)}</div></div> }
