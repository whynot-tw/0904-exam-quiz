...
import { batchUpdateCmsQuestionSubcategories, getCmsQuestions, getCmsSettings, getStarredQuestions, getStarredQuestionStats, getUserAnswerRows, getUserAttempts, getUserLearningGoal, getWrongQuestions, recordAttempt, toggleStarredQuestion, updateCmsQuestion, updateCmsQuestionSubcategory, updateStarredQuestionReminder, updateStarredQuestionTag, updateUserLearningGoal } from "./db";
...
export const appRouter = router({
  ...
  quiz: router({
    ...
    adminList: adminProcedure.input(z.object({ needsReviewOnly: z.boolean().default(false), subcategoryReviewOnly: z.boolean().default(false) })).query(async ({ input }) => { const cmsRows = await getCmsQuestions(); const list = cmsRows.length ? cmsRows.map(cmsQuestionToQuizQuestion) : getQuizQuestions(); const filtered = input.needsReviewOnly ? list.filter(question => question.import_status === "needs_review") : input.subcategoryReviewOnly ? list.filter(question => question.subcategory_status === "needs_manual_review") : list; return filtered.map(toClientQuestion); }),
    ...
    adminBatchUpdateSubcategory: adminProcedure.input(z.object({ questionIds: z.array(z.string().min(1)).min(1).max(234), subcategory: z.string().trim().min(1).max(80), subcategoryStatus: z.enum(["assigned", "needs_manual_review"]), subcategoryNotes: z.string().trim().max(500).optional() })).mutation(async ({ input }) => {
      const updatedCount = await batchUpdateCmsQuestionSubcategories(input.questionIds, input);
      return { success: true, updatedCount, persistedTo: "cms-database" as const };
    }),
  }),
  attempts: router({
