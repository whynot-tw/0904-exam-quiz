...
describe("錯題本", () => {
  it("拒絕未登入使用者讀取個人錯題清單", async () => {
    ...
    await expect(caller.wrongQuestions.markMastered({ questionId: "HARDWARE-1" })).rejects.toMatchObject({ code: "UNAUTHORIZED" });
  ...
  it("自動記錄答錯題目、提供個人清單，並在連續答對兩次後標記為已熟悉", async () => {
    const db = await getDb();
      ...
      try {
        if (db) {
          await db.delete(wrongQuestionConciseExplanations).where(eq(wrongQuestionConciseExplanations.userId, learnerId));
      ...
      const fetchMock = vi.fn(async () => new Response(JSON.stringify({ id: "mock-ai", created: 0, model: "gpt-5-mini", choices: [{ index: 0, message: { role: "assistant", content: JSON.stringify({ errorReason: "最近選項與官方正解不同", whyItMatters: "辨識關鍵條件", correctThinking: "依官方答案逐項排除", reviewTip: "先背官方關鍵字", sourceNotice: "本補充以官方題庫內容為準。", summary: "先找出官方正解的關鍵條件。", memoryTip: "先背官方關鍵字，再排除不符選項。", headline: "優先複習此範圍", overallAssessment: "目前錯題需要持續複習。", priorityTopics: [{ topic: "電腦硬體與組裝", evidence: "依個人錯題統計。", advice: "依官方解析重做。" }], reviewPlan: ["重讀官方解析", "進行錯題重刷", "隔日再複習"] }) }, finish_reason: "stop" }] }), { status: 200, headers: { "content-type": "application/json" } }));
      const originalFetch = globalThis.fetch;
      vi.stubGlobal("fetch", fetchMock);
      try {
        const caller = appRouter.createCaller(learnerContext());
        expect(await caller.wrongQuestions.conciseList()).toEqual([]);
        const batch = await caller.wrongQuestions.generateConciseBatch();
        expect(batch).toMatchObject({ requestedCount: 1, generatedCount: 1, failedCount: 0, remainingCount: 0 });
        expect(await caller.wrongQuestions.conciseList()).toEqual(expect.arrayContaining([expect.objectContaining({ questionId, summary: "先找出官方正解的關鍵條件。", memoryTip: "先背官方關鍵字，再排除不符選項。", generationCount: 1 })]));
        const helpful = await caller.wrongQuestions.rateConcise({ questionId, feedback: "helpful" });
        ...
        expect(fetchMock.mock.calls.some(call => String(call[1]?.body).includes("官方正解"))).toBe(true);
        ...
        expect(fetchMock.mock.calls.some(call => String(call[1]?.body).includes("使用者最近選擇：A"))).toBe(true);
        expect(fetchMock.mock.calls.some(call => String(call[1]?.body).includes("max_completion_tokens"))).toBe(true);
      } finally {
